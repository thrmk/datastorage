...
class DeviceModel(db.Model):
    ...
    devname = db.Column(db.String(15),primary_key=True)
    ...
    @classmethod
    def find_by_name(cls, stamp,devname):
        return cls.query.filter_by(stamp=stamp,devname=devname).first()
...
class Device(Resource):
    ...
    def get(self, stamp,devname):
        device = DeviceModel.find_by_name(stamp,devname)
        if device:
            return device.json()
        return {'message': 'Device not found'}, 404

    def post(self, stamp,devname):
        #global reading
        #reading=reading+1
        #devId1 =devId1 +1
        #count =str(devId1)
        ...
        if device is None:
            device = DeviceModel(stamp,devname,data['tStamp'],data['rphvol'],data['yphvol'],data['bphvol'])
            print("device=",device)
