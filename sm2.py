...
class Device(Resource):
    ...
    parser.add_argument('tStamp',type=str)
    parser.add_argument('rphvol',type=float)
    parser.add_argument('yphvol',type=float)
    parser.add_argument('bphvol',type=float)
    ...
    def post(self, stamp):
        ...
        if device is None:
            ...
            print("device=",device)
        elif device is None and devname!=stamp:
            device = DeviceModel(stamp,data['devname'],data['tStamp'],data['rphvol'],data['yphvol'],data['bphvol'])
            print("device=",device)
...
flag=True
engine = db2.create_engine('sqlite:///data.db')
connection = engine.connect()
metadata = db2.MetaData()
voltage= db2.Table('devices', metadata, autoload=True, autoload_with=engine)
   #Equivalent to 'SELECT * FROM census'
query = db2.select([voltage])
print("query=",query)
...
#x=db.select([db.func.sum(voltage.columns.rphvol)]).where(voltage.columns.devname == 'dev_01')
#print(x)
print(df.rphvol.where(df.devname=='dev_01'))
print(df.tStamp)
...
app.layout = html.Div([navbar,content,data1,graph(),dcc.Location(id="url",refresh=True)])
